...
# 7. Broom and regression diagnostics
...
# To reorganize data.frame columns
# it can be easier to use numbers than
# names. The function pair column numbers
# names to simplify the process.
...
# Here medv, median home value
# is the dependent variable and we
# choose to put if first.

Boston1 <- Boston[,c(14,1:13)]

# We are going to make a scatterplot
# matrix, to look at the relationship
# between pairs a variable.  We start
# by looking at the density. we can do
# thi by putting density plots on the
# diagaonusing bivariate hexagon binning
# off diagonals.
...
The plot may be hidden behind the RStudio screen
...
#
# We can see that rad and tax variance estimates are highly
# inflated.  We could see the high correlations (.91) in the
# correlation plot.
#
# One approach to the problem is the principal components
# of the explanatory variables.  The linear combinations
# of the explanatory variables are linearly independent
# which also means they are not correlated.  We pick
# enough components to capture most of the variation present
# in the explanatory variables.  The hope is that
# the omitted variation is noise or structure of little
# importance.
#
# Another lecture and assignment addresses principal components.

# 7. Broom and regression diagnostics

lm.fig6Fort <- fortify(lm.fit6)
head(lm.fig6Fort)
